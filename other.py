        ...
        while i > 0 or j > 0:
            ...
            if i > 0 and j > 0:
                if s1[i-1] == s2[j-1]:  # Match
                    if current_score == scoring_matrix[i-1][j-1] + match:
                        aligned_s1.append(s1[i-1])
                        aligned_s2.append(s2[j-1])
                        i -= 1
                        j -= 1
                else:  # Mismatch
                    if current_score == scoring_matrix[i-1][j-1] + mismatch:
                        left_score = scoring_matrix[i][j-1] if j > 0 else None
                        up_score = scoring_matrix[i-1][j] if i > 0 else None

                        # Choose largest value to go left or up
                        if left_score is not None and (up_score is None or left_score > up_score):  
                            aligned_s1.append('-')
                            aligned_s2.append(s2[j-1])
                            j -= 1
                        else:  
            ...
            elif i > 0 and (current_score == scoring_matrix[i-1][j] + gap_penalty):  # Gap in s2
                aligned_s1.append(s1[i-1])
                aligned_s2.append('-')
                i -= 1

            elif j > 0 and (current_score == scoring_matrix[i][j-1] + gap_penalty):  # Gap in s1
                aligned_s1.append('-')
                aligned_s2.append(s2[j-1])
                j -= 1
